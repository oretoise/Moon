...
This seems to have returned ALL users for the site, including one with is_admin marked True. For authenitcation, we'll worry about the usernames and passwords. So let's take the response we got from Burp, clean it up, and see if HashID can make any sense of the password hashes.
...
### John
To get John working properly, we'll create a text file with the usernames and hashes organized like so: user:hash .
